...
// 性能需求：虽然两者在性能上都很优秀，根据具体的性能需求和优化可能性，我可能会对这两个框架进行更深入的比较。

// 总的来说，没有绝对的“最好”，只有最适合当前情境的选择。 2. react hooks 诞生的原因是什么？为了解决什么问题
// React Hooks 是在 React 16.8 版本中引入的一个重要特性，它们的诞生主要是为了解决以下几个问题：

// 复杂组件逻辑难以重用：在 Hooks 之前，如果你想在多个组件之间重用一些状态逻辑，你可能需要用到高阶组件（HOCs）或者渲染属性（render props）等模式。这些模式可以工作，但它们往往使得组件层次深且难以理解，也增加了代码的复杂度。

// 难以理解的类组件：类组件让许多开发者感到困惑，特别是对于初学者来说，需要理解 JavaScript 中的 this 关键字的工作方式，以及如何正确地绑定事件处理器方法。此外，类组件的生命周期方法经常让人头疼，尤其是当需要在不同生命周期方法中进行相同逻辑处理时。

// 副作用代码分散：在类组件中，相关的业务逻辑代码往往被分散在多个生命周期方法中（比如 componentDidMount 和 componentDidUpdate 中都需要进行数据的加载操作），这使得逻辑难以跟踪和维护。

// 复杂的组件状态逻辑：类组件中状态逻辑经常和 UI 逻辑紧密耦合在一起，这使得状态逻辑难以测试和重用。
...
// Hooks 的设计使得状态和相关逻辑可以更容易地打包在一起，从而更易于重用和测试，并且可以在不改变组件层次结构的情况下共享。这些特性使得开发者可以写出更简洁、更易于维护的代码。 3. hooks 带来了哪些新的问题？编写时如何注意
...
// 通过理解 Hooks 的工作原理并遵循最佳实践，可以最大限度地减少这些问题。始终关注 React 的官方文档和社区，以获取关于 Hooks 使用的最新指南和技巧。 4. Class 组件和 Function 组件的优劣对比
// React 中的类组件和函数组件各有优缺点，随着 React 的发展，这些差异也在不断演变。以下是它们的一些对比：
// 类组件:
// 优势:

// 状态管理: 在引入 Hook 之前，类组件是唯一可以持有状态(state)的组件。
// 生命周期方法: 提供丰富的生命周期方法（如 componentDidMount, componentDidUpdate 等），允许执行复杂的操作。
// 可以使用 this 关键字: 访问或修改组件状态和属性。

// 劣势:

// 更复杂的语法: 类组件需要理解 JavaScript 中的 this 以及如何正确绑定事件处理函数。
// 更大的尺寸: 类组件通常比函数组件大小要大，可能导致更大的包体积。
// 性能开销: 类组件可能有轻微的性能开销，因为它们需要实例化。

// 函数组件:
// 优势:

// 简洁性: 语法更简洁，没有 this 的复杂性，容易编写和理解。
// 轻量级: 通常体积比类组件小，意味着更快的加载和解析。
// 函数式编程: 更容易使用函数式编程范式，代码可能更清晰和易于测试。
// Hooks: 随着 React Hooks 的引入，函数组件几乎可以做到类组件的所有事情，并且能够更好地利用 React 的特性，如状态管理和副作用。

// 劣势:

// 生命周期管理: 在 Hooks 出现之前，函数组件没有直接对应于类组件生命周期方法的概念。
// 学习成本: 对于习惯于使用类的开发者来说，切换到函数组件加上 Hooks 的模式可能需要一段时间来适应。
...
// 选择类组件还是函数组件通常取决于个人偏好、项目需求和团队规范。随着时间的推移和 React 的发展，函数组件加 Hooks 的模式正在成为社区中的主流写法。 5. 说一下 hooks 的闭包问题
...
// javascript 复制代码 function MyComponent() {
// const [count, setCount] = useState(0);

// useEffect(() => {
// const intervalId = setInterval(() => {
// // 这里的 count 值是这个闭包创建时的值，而不是最新的 count 值
// console.log(count);
// }, 1000);

//     return () => clearInterval(intervalId);

// }, []); // 空依赖数组意味着 effect 只会在挂载时运行
...
// javascript 复制代码 setCount((currentCount) => currentCount + 1);

// 使用引用（useRef）

// 使用 useRef 来保持对最新值的引用：

// javascript 复制代码 const countRef = useRef(count);
// countRef.current = count;

// useEffect(() => {
// const intervalId = setInterval(() => {
// // 使用 countRef.current 总是获取到最新的 count 值
...
// 在 useEffect 中包含所有依赖项

// 通过在 useEffect 的依赖数组中包含所有相关的依赖项，确保在依赖项更新时重新运行 effect：

// javascript 复制代码 useEffect(() => {
// // ...
// }, [count]); // count 包含在依赖数组中，这样每次 count 更新时都会重新设置定时器

// 使用函数式更新

// 对于设置状态的函数，传递一个函数而不是具体的值，这个函数接受前一个状态值作为参数，返回新的状态值。

// javascript 复制代码 setCount((prevCount) => prevCount + 1);

// 理解和利用闭包是高级 JavaScript 和 React 开发的关键。通过合适的解决方案，可以确保我们的 Hooks 正确地引用和更新状态，避免产生难以追踪的 bug。 6. function 组件每次渲染都会生新执行，useState/useRef 如何保存值不被清除
...
//    javascript 复制代码 const [count, setCount] = useState(0); // 始终保持对同一状态的引用
...
//    javascript 复制代码 const myRef = useRef(initialValue); // myRef.current 将在渲染间保持不变
...
// 总结
// 总的来说，React 的这些 Hooks 的工作原理依赖于 React 的 Fiber 架构，该架构允许 React 在内部追踪组件状态和引用。每当组件渲染时，它都会以正确的顺序和位置检索状态和引用，确保它们在多次渲染之间保持不变。这个设计使得函数组件可以在不失去其函数式和声明式特性的同时，使状态和引用在组件的整个生命周期中保持持久化。 7. 什么是 Fiber?诞生背景是什么
...
// 新的生命周期方法：为了适应 Fiber 架构，React 引入了新的生命周期方法（如 getDerivedStateFromProps），这些方法适用于新的渲染策略。
...
// 总之，Fiber 的引入使得 React 更加强大和高效，特别是在处理大规模应用和复杂更新时。它为未来的 React 特性和优化提供了基础，包括异步渲染和并发模式（Concurrent Mode），这些都是 React 应用未来性能提升的关键方面。 7. 任务优先级如何处理的？底层模型是什么
...
// requestIdleCallback：这是浏览器 API 的一部分，允许开发者安排在主线程空闲时运行的低优先级工作。React Scheduler 使用了类似 requestIdleCallback 的机制，用于在浏览器空闲时执行低优先级工作。

// requestAnimationFrame：这是另一种浏览器 API，常用于在下次重绘前执行动画或视觉更新。React Scheduler 利用它来确保高优先级的更新能够及时执行。
...
// 总结
// React Fiber 的任务优先级处理允许 React 应用更高效地管理和更新状态，避免长时间占用主线程，这样就可以保持应用的流畅性和快速响应用户交互。优先级调度是 Fiber 架构的核心特性之一，使得 React 可以在复杂应用中实现更好的性能和用户体验。 8. 这种模型下，如何实现任务中断的？用了什么技术
...
// 时间分片（Time Slicing）
// React 使用浏览器的 requestIdleCallback 和 requestAnimationFrameAPI 来执行时间分片。使用 requestIdleCallback 可以让 React 在浏览器空闲时执行低优先级的更新，而 requestAnimationFrame 则用于高优先级的更新，如动画。
// 任务中断
...
// 总结
// 通过上述的技术和架构，React 的 Fiber 架构能够在保持 UI 响应的同时，进行复杂的渲染任务。这使得 React 应用即使在繁重的计算和大量的组件更新时，也能保持良好的性能表现。任务中断是实现这一目标的关键技术之一。 9. 调度示例代码
...
// // 使用 requestAnimationFrame 和 MessageChannel 模拟实现任务循环
...
// 注意，这只是一个非常简化的示例，仅用于阐释概念。React 的实际调度器要复杂得多，并且考虑了更多的性能优化和边缘情况处理。React 官方的 scheduler 包（@scheduler）中包含了 React 团队用于任务调度的实现细节。 10. 使用 MessageChannel 是为了解决什么问题
...
// 6. MessageChannel 和 web worker 的区别
...
// 在某些情况下，它们可以一起使用，例如，你可以通过 MessageChannel 将一个 MessagePort 传递给一个 Web Worker，然后在 Worker 和主线程之间建立直接的通信通道。这样可以避免使用全局 postMessage 接口，并提供更细粒度的通信控制。 12. 为什么不用 Promise 而是用 MessageChannel
...
// // 使用 MessageChannel 来触发任务
// const channel = new MessageChannel();
// const { port1, port2 } = channel;

// // 空闲时间的阈值
// const IDLE_THRESHOLD = 50; // 50 毫秒
...
// // 使用 requestAnimationFrame 来获取下一帧的开始时间
...
// 这个例子并不是 React 实际使用的代码，但它展示了 React 调度器可能使用的一些基本概念。实际的 React 调度器实现要复杂得多，它需要处理各种边缘情况、优先级、错误边界等复杂的逻辑。 14. 讲一下 Concurrent Mode
...
// 中断渲染: 在 Concurrent Mode 中，渲染工作可以被中断并稍后再继续。这意味着 React 可以根据用户交互或其他重要任务暂停正在进行的渲染工作，以确保主线程的响应性。

// 优先级化更新: React 可以为更新分配不同的优先级。例如，来自用户输入的更新（如键盘或点击事件）可以被赋予更高的优先级，而数据抓取更新可以被赋予较低的优先级。

// 并发数据抓取: 开发者可以使用 Suspense 组件来指定组件如何等待异步数据。这允许 React 在数据抓取时保持 UI 的响应性，甚至在数据到达前开始渲染 UI。

// 避免水平渲染: 当数据缺失会导致组件抛出错误时，Suspense 可以用于定义一个加载状态，防止应用进入一个不完整的、中间状态。
...
// javascript 复制代码 import React from "react";
...
// Concurrent Mode 还没有完全成熟并在 React 的所有版本中默认启用，因为它可能引入一些不兼容的更改和新的概念，如 Suspense。开发者在使用时需要权衡其带来的好处和潜在的重构工作。 15. 讲一下 react18 的批处理
...
// jsx 复制代码 import { flushSync } from "react-dom";
...
// 总结
// React 18 中的自动批处理是对 React 更新机制的显著改进，它扩展了批处理可应用的范围，并提高了应用性能。开发者无需更改现有代码就可以从中受益，但同时也拥有了必要时控制更新同步的能力。该特性的引入使得 React 应用可以更有效率地处理状态更新，尤其是在复杂的应用和异步操作中。 16. 说下 useInsertionEffect、useEffect、useLayoutEffect 区别
...
// 在编写组件时，大多数副作用应该使用 useEffect。只有当你需要同步读取或更改 DOM，或者当你需要在绘制前注入样式时，才考虑使用 useLayoutEffect 或 useInsertionEffect。 17. 讲一下 react 状态发生变化触发视图更新的链路
...
//    javascript 复制代码 const [state, setState] = useState(initialState);

// // 状态更新
// setState(newState);

// 当状态更新被触发时，React 会将该组件标记为需要更新。 2. 调度更新
// React 的调度器接收到更新请求，并根据其优先级决定何时执行更新。在 Concurrent Mode 下，React 可能会根据任务的紧急程度和浏览器的空闲时间来推迟或中断更新。 3. 协调（Reconciliation）
// 协调过程开始时，React 会为当前的更新创建一个工作单元，并开始遍历组件树（也称为 Fiber 树）。

// 比较阶段：React 会比较新的组件状态和上一次渲染的状态，计算出实际需要变更的部分。
// 生成 Fiber 树：React 为渲染中的每个组件创建或更新 Fiber 节点。Fiber 是 React 用于跟踪组件树中的每个组件状态和结构的内部数据结构。
...
// 创建新的虚拟 DOM 树：当组件的状态或 props 发生变化时，React 会创建一棵新的虚拟 DOM 树。

// 比较新旧虚拟 DOM 树：React 使用 diff 算法比较新旧虚拟 DOM 树中的元素。这个比较过程是递归的，从根组件开始，一直到所有子孙组件。

// 确定更新的节点：在比较新旧虚拟 DOM 时，React 会识别哪些元素发生了变化。例如，如果一个组件的类型变了（从<div>变成<span>），或者 key 属性不同了，React 会摧毁旧组件并构建新组件。如果组件类型相同，React 会比较这个组件的 props，并确定需要更新哪些属性。

// 标记需要更新的实际 DOM：React 为每个发生变化的虚拟 DOM 元素生成更新操作，并标记它们在实际 DOM 中对应的节点需要更新。
...
// 协调阶段是 React 内部的实现细节，而作为开发者，我们通常不需要直接与之交互。理解这个过程有助于我们编写更高效的组件，例如，通过合理使用 keys、避免不必要的重渲染等。 19. 实现一个简单版本的 diff 算法
...
// javascript 复制代码 function diff(oldVTree, newVTree) {
// // 存放差异的对象
// const patches = {};
...
// 需要注意的是，这个简化版本的 diff 算法并不包含 React 实际使用的所有优化和特性。例如，它没有处理列表中元素排序和移动的情况，也没有实现复杂的 keys 处理逻辑，这些都是 React diff 算法的重要组成部分。此外，实际的算法还需要考虑各种边缘情况和性能优化策略。 20. StrictMode 为什么会渲染两次？它主要为了解决什么问题
...
// 总之，StrictMode 和它的双重渲染机制是为了帮助开发者在未来的 React 版本中保持应用的稳定性和性能，特别是面对即将到来的并发特性时。通过提前暴露和解决这些问题，应用将能够更平滑地适应 React 的更新和改进。 21. react 有哪些工具可以去定位 react 的性能问题
...
// Profiler（性能分析器）: React DevTools 中的 Profiler 选项卡可以记录和查看组件渲染的性能。它可以显示哪些组件重新渲染以及每次渲染的时间。
// Highlight Updates: 可以高亮显示在渲染时发生更新的组件，这有助于发现不必要的重新渲染。
...
// 所有这些工具和技术都能帮助你诊断 React 应用中的性能瓶颈。通常，你需要结合使用它们来获取最全面的性能分析，并据此进行优化。 22. 性能工具使用示例
...
// jsx 复制代码 import React, { Profiler } from "react";

// function onRenderCallback(
// id, // 发生提交的 Profiler 树的 “id”
// phase, // "mount"（如果树刚刚挂载）或者 "update"（如果树重新渲染）
// actualDuration, // 渲染提交花费的时间
// baseDuration, // 估算不使用 memoization 的情况下渲染整颗子树需要的时间
// startTime, // 本次更新中 React 开始渲染的时间
// commitTime, // 本次更新中 React 提交更改的时间
// interactions // 本次更新涉及的 interaction 集合
// ) {
// // 处理渲染信息或记录日志
// }
...
// sh 复制代码 npm install web-vitals
// // 或者
// yarn add web-vitals

// 记录核心 Web Vitals 指标:
// 在应用的主要入口点，集成 web-vitals 库并记录性能指标：
// javascript 复制代码 import { getCLS, getFID, getLCP } from "web-vitals";
...
// sh 复制代码 npm install @welldone-software/why-did-you-render
// // 或者
